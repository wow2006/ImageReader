...
static bool checkJPEG(const uchar header[4]) {
...
static bool checkPNG(const uchar header[8]) {
...
static bool checkGIF(const uchar header[4]) {
...
static bool checkTIF(const uchar header[4]) {
...
BaseImage::BaseImage(const std::string &_imageName) { open(_imageName); }

bool BaseImage::open(const std::string &_imageName) {
...
bool BaseImage::save(const std::string &_imageName, const int JPEG_QUALITY) {
  std::unique_ptr<Encoder::EncoderInterface> encoder =
      ImageFormatUtility::getEncoder(_imageName);
  if (encoder) {
    std::vector<uchar> outputData;
    std::size_t fileSize = 0;
    encoder->encode(mImagePtr.data(), mWidth, mHeight, mChannels, outputData,
                    fileSize, JPEG_QUALITY);
    writeImage(_imageName, reinterpret_cast<char *>(outputData.data()),
               fileSize);
    return true;
  }
  return false;
